while True :
    ...
    print("name_status 1 :",name_status)

    for i in range(len(name_status)) :
        name_status[i].append(result[i])
    #lst.sort(key=lambda x:x[0])
    print("name_status 2 :",name_status)
    print("\n\n\n\n\n\n")
    ...
    print("\n\n\n\n\n\n")
    print("tier_weapons 1:",tier_weapons)
    tier_weapons.sort(key=lambda x:x[1] ,reverse=True)
    print("tier_weapons 2:",tier_weapons)
    ...
    for i in range(len(tier_weapons)) :
        ...
        for j in range(len(tier)) :
            ...
            if si == str(tier[j][0]).strip().lower() and (int(tier_weapons[i][2]) == int(tier[j][1])) :
                #print("\n"*20,True,"\n"*20)

                right_tier_count += 1
    print(right_tier_count)
    f = open("result.txt","a")    
    f.write(str(right_tier_count)+" ")
    f.close()
    if 10 <= right_tier_count :
        print(right_tier_count)
        print(form_)
        break
    print("\n\n")
